...
chrome.storage.local.get(["latestRecommendations"]).then(async (result) => {
    ...
    if (recommendations) {
        for (let i = 0; i < recommendations.length; i++) {
            ...
            let ratings = (await chrome.storage.local.get(["recommendationRatings"]))
                .recommendationRatings || {};
            const activeUrls = recommendations.map((rec) => rec.url);
            ...
            let starsHtml = '<div class="star-rating" data-url="' + recommendationKey + '">';
            for (let star = 1; star <= 5; star++) {
                starsHtml += `<span class="star" data-star="${star}" style="cursor:pointer; color:${star <= currentRating ? "#FFD700" : "#ccc"}">&#9733;</span>`;
            }
            starsHtml += "</div>";
            const elementString = `
