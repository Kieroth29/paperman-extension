...
export async function getRecommendationsAlarm(): Promise<Array<Publication>> {
  ...
  if (userId) {
    ...
    if (response.status === 200) {
      const data = <PapermanAPIResponse>await response.json();
...
async function orderLatestRecommendations(
  requestRecommendations: Publication[]
) {
  let { latestRecommendations } = await chrome.storage.local.get(
